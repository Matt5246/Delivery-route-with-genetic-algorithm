...
cost_matrix = np.array([[np.linalg.norm(locations[i] - locations[j]) for j in range(len(customers))] for i in range(len(customers))])
...
def plot_route(route):
    ...
    route_locations.append(route_locations[0])  # Zamknięcie trasy
    ...
    plt.title('Optymalna Trasa Dostaw')
